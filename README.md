...
**Причина:** Telegram WebApp не загружается или initData отсутствует
...
1. **Проверьте настройки в BotFather:**
   ...
   /setmenubutton
   ```
   Убедитесь, что URL указан правильно (HTTPS)

2. **Проверьте логи в консоли:**
   - Откройте DevTools (F12)
   - Ищите сообщения: "🔍 Checking for Telegram WebApp..."
   - Если видите "❌ Telegram WebApp not found", проблема в настройке

3. **Тестируйте локально:**
   - Откройте `http://localhost:3000/test.html`
   - Нажмите "Проверить WebApp"
   - Если WebApp не найден, проверьте настройки Telegram

4. **Проверьте URL:**
   - Мини-приложение должно открываться по HTTPS
   - Проверьте, что домен добавлен в список разрешенных в BotFather

### "Проверяем доступ..." (зависает)

**Причина:** Ожидание загрузки Telegram WebApp

**Решение:**
1. Подождите 5-10 секунд
2. Проверьте интернет-соединение
3. Попробуйте перезапустить мини-приложение
4. Очистите кэш браузера

### Логи показывают "WebApp found" но все равно ошибка

**Причина:** initData пустой или поврежденный

**Решение:**
1. Проверьте токен бота на корректность
2. Убедитесь, что пользователь авторизован
3. Попробуйте выйти и войти снова в Telegram
